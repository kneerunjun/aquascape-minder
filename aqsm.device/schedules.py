...
import json, sys, datetime, pdb, hardware
...
def turn_on_led():
    if hardware.relay_state(hardware.LEDGPIO)==0:
        hardware.relay(hardware.LEDGPIO,1)
        print("Turned on the LED now")
@sched.scheduled_job('cron', hour=led_off_schedule["hours"], minute=led_off_schedule["minutes"], timezone="Asia/Kolkata")
def turn_off_led():
    if hardware.relay_state(hardware.LEDGPIO)==1:
        hardware.relay(hardware.LEDGPIO,0)
        print("Turned off the LED now")
...
def turn_on_air():
    if hardware.relay_state(hardware.AIRGPIO)==0:
        hardware.relay(hardware.AIRGPIO,1)
        print("turned on the airpump")

@sched.scheduled_job('cron', hour=air_off_schedule["hours"], minute=air_off_schedule["minutes"], timezone="Asia/Kolkata")
def turn_off_air():
    if hardware.relay_state(hardware.AIRGPIO)==1:
        hardware.relay(hardware.AIRGPIO,0)
        print("Turned off the airpump now")
...
def turn_on_filter():
    if hardware.relay_state(hardware.FLTGPIO)==0:
        hardware.relay(hardware.FLTGPIO,1)
        print("turned on the filter")

@sched.scheduled_job('cron', hour=filter_off_schedule["hours"], minute=filter_off_schedule["minutes"], timezone="Asia/Kolkata")
def turn_off_filter():
    if hardware.relay_state(hardware.FLTGPIO)==1:
        hardware.relay(hardware.FLTGPIO,0)
        print("Turned off the airpump now")
...
def turn_on_feeder():
    if hardware.relay_state(hardware.FEEDGPIO)==0:
        hardware.relay(hardware.FEEDGPIO,1)
        print("Turned on the feeder now")

@sched.scheduled_job('cron', hour=feeder_off_schedule["hours"], minute=feeder_off_schedule["minutes"], timezone="Asia/Kolkata")
def turn_off_feeder():
    if hardware.relay_state(hardware.FEEDGPIO)==1:
        hardware.relay(hardware.FEEDGPIO,0)
        print("Turned off the feeder now")
